...
const CommentsScreen = ({
  ...
  const loadComments = useCallback(async (): Promise<void> => {
    ...
    try {
      const cardComments = await cardCommentsService.getComments(
        parseInt(cardId),
      );
      ...
      toast({
        ...
        description:
          error.response?.data?.detail || t("card.loadCommentsError"),
        variant: "destructive",
      });
    }
  }, [cardId, toast, t]);
  ...
  const addComment = async (): Promise<void> => {
    ...
    setLoading(true);
    try {
      const newComment = await cardCommentsService.createComment(
        parseInt(cardId),
        text,
      );
  ...
  const saveCommentEdit = async (): Promise<void> => {
    ...
    setLoading(true);
    try {
      const updatedComment = await cardCommentsService.updateComment(
        editingCommentId,
        text,
      );
      ...
      toast({
        title: t("common.error"),
        description:
          error.response?.data?.detail || t("card.updateCommentError"),
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const deleteComment = async (commentId: number): Promise<void> => {
    ...
    try {
      ...
      toast({
        title: t("common.error"),
        description:
          error.response?.data?.detail || t("card.deleteCommentError"),
        variant: "destructive",
      });
    } finally {
      setLoading(false);
                ...
                {permissions.canEditComment(comment) && (
                  <div className="flex gap-1">
                    ...
                    <button
                      ...
                      disabled={
                        loading || !permissions.canDeleteComment(comment)
                      }
