...
const CardDetail = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !target.closest(".priority-dropdown-container")
      ) {
      ...
      if (
        ...
        !target.closest(".assignee-dropdown-container")
      ) {
    ...
    try {
      ...
      if (isNewCard && initialData) {
        ...
        const initialFormData = {
          ...
          list_id:
            initialData.list_id ||
            (listsData.length > 0 ? listsData[0].id : -1),
        };
        ...
        setFormData({
          ...
          label_ids:
            proposedChanges.label_ids ?? (card.labels?.map((l) => l.id) || []),
  ...
  const hasUnsavedChanges = () => {
    ...
    const formChanged =
      ...
      JSON.stringify(formData.label_ids.sort()) !==
        JSON.stringify(originalFormData.label_ids.sort());

    // Check checklist changes
    const checklistChanged =
      JSON.stringify(checklist) !== JSON.stringify(originalChecklist);
          ...
          <div className="space-y-2 relative list-dropdown-container">
            ...
            >
              <button
                type="button"
                onClick={() =>
                  !isViewOnly && setShowListDropdown(!showListDropdown)
                }
                disabled={isViewOnly}
          ...
          <div className="space-y-2 relative priority-dropdown-container">
            ...
            >
              <button
                type="button"
                onClick={() =>
                  !isViewOnly && setShowPriorityDropdown(!showPriorityDropdown)
                }
                disabled={isViewOnly}
                className="w-full btn-touch bg-card border-2 border-border rounded-lg px-4 text-foreground disabled:opacity-50 flex items-center justify-between"
              >
          ...
          <div className="space-y-2 relative assignee-dropdown-container">
            ...
            >
              <button
                type="button"
                onClick={() =>
                  !isViewOnly && setShowAssigneeDropdown(!showAssigneeDropdown)
                }
                disabled={isViewOnly}
                className="w-full btn-touch bg-card border-2 border-border rounded-lg px-4 text-foreground disabled:opacity-50 flex items-center justify-between"
              >
