...
export const useElasticTransition = ({
  ...
  useLayoutEffect(() => {
    ...
    const elems = Array.from(nodeList).filter((el) =>
      (el as HTMLElement).getAttribute("data-card-id"),
    ) as HTMLElement[];
  ...
  useEffect(() => {
    ...
    const cardElements = Array.from(nodeList).filter((el) =>
      (el as HTMLElement).getAttribute("data-card-id"),
    ) as HTMLElement[];
    ...
    if (
      ...
      try {
        const colEl = containerRef.current?.closest(
          "[data-list-id]",
        ) as HTMLElement | null;
        if (colEl) {
          const listId = Number(colEl.getAttribute("data-list-id"));
          const parentSuppress =
            (window as any).__SUPPRESS_ANIM_LIST_ID || null;
      ...
      requestAnimationFrame(() => {
        ...
        requestAnimationFrame(() => {
          ...
          const columnEl =
            (container.closest(".kanban-column") as HTMLElement) || container;
    ...
    const currentIds = cardElements.map((el) =>
    ...
    const removedFromOriginal = originalIds.filter(
      (id) => !currentIds.includes(id),
    );

    const effectiveRemoved =
      removedFromOriginal.length > 0 ? removedFromOriginal : removedIds;
    ...
    if (isDragging && effectiveRemoved.length > 0) {
      ...
      movingCards.forEach((el) => {
        const id = el.getAttribute("data-card-id")!;
        const first =
          (originalPositions && originalPositions.get(id)) ||
          prevRects!.get(id);
    ...
    if (
      isDragging &&
      dropTarget === null &&
      previousDropTarget.current !== null
    ) {
