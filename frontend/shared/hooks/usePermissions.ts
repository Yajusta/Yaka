...
export const usePermissions = (user?: User | null) => {
  ...
  return useMemo(
    () => ({
      ...
      canModifyCard: (card?: Card) =>
        permissions.canModifyCard(userRole, card, userId),
      canModifyCardMetadata: (card?: Card) =>
        permissions.canModifyCardMetadata(userRole, card, userId),
      canModifyCardContent: (card?: Card) =>
        permissions.canModifyCardContent(userRole, card, userId),
      canMoveCard: (card?: Card) =>
        permissions.canMoveCard(userRole, card, userId),
      canDeleteCard: permissions.canDeleteCard(userRole),
      canArchiveCard: permissions.canArchiveCard(userRole),
      canAssignCard: (card?: Card) =>
        permissions.canAssignCard(userRole, card, userId),

      // Comment permissions
      canCommentOnCard: permissions.canCommentOnCard(userRole),
      canEditComment: (comment?: CardComment) =>
        permissions.canEditComment(userRole, comment, userId),
      canDeleteComment: (comment?: CardComment) =>
        permissions.canDeleteComment(userRole, comment, userId),

      // Checklist item permissions
      canCreateCardItem: (card?: Card) =>
        permissions.canCreateCardItem(userRole, card, userId),
      canToggleCardItem: (card?: Card) =>
        permissions.canToggleCardItem(userRole, card, userId),
      canModifyCardItem: (card?: Card) =>
        permissions.canModifyCardItem(userRole, card, userId),
      canDeleteCardItem: (card?: Card) =>
        permissions.canDeleteCardItem(userRole, card, userId),
