...
const handleApiError = (error: any): never => {
  ...
  switch (status) {
    ...
    case 409:
      throw new CardsApiError(
        detail ||
          "Conflit détecté. Cette action ne peut pas être effectuée en raison d'un conflit avec l'état actuel.",
        status,
        "CONFLICT",
      );
    case 422:
...
export const cardsApi = {
  ...
  async getCardsByList(listId: number): Promise<Card[]> {
    try {
      const response: AxiosResponse<Card[]> = await api.get(
        `/cards/?list_id=${listId}&include_archived=false`,
      );
  ...
  async moveCard(
    ...
    try {
      ...
      const response: AxiosResponse<Card> = await api.patch(
        `/cards/${cardId}/move`,
        moveRequest,
      );
      return response.data;
