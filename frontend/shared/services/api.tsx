...
export const authService = {
  async login(email: string, password: string): Promise<User> {
    ...
    const response = await apiInstance.post<{ access_token: string }>(
      "/auth/login",
      formData,
      {
        headers: {
  ...
  setCurrentUserToStorage(user: User): void {
    try {
      ...
      if (!user || typeof user !== "object" || !("id" in user)) {
        console.error(
          "Tentative de sauvegarde d'un utilisateur invalide:",
          user,
        );
        return;
      }
      ...
      try {
        ...
        console.error(
          "Erreur lors du nettoyage du localStorage:",
          cleanupError,
        );
      }
    }
  ...
  async checkAIFeatures(): Promise<{ ai_available: boolean }> {
    const response = await getApiInstance().get<{ ai_available: boolean }>(
      "/auth/ai-features",
    );
...
export const userService = {
  ...
  async inviteUser(payload: {
    ...
    const response = await getApiInstance().post<User>("/users/invite", {
      email: payload.email,
  ...
  async updateUser(userId: number, userData: Partial<User>): Promise<User> {
    const response = await getApiInstance().put<User>(
      `/users/${userId}`,
      userData,
    );
  ...
  async updateLanguage(language: string): Promise<User> {
    const response = await getApiInstance().put<User>("/users/me/language", {
      language,
    });
...
export const cardService = {
  async getCards(filters: Filters = {}): Promise<Card[]> {
    ...
    const response = await getApiInstance().get<Card[]>(
  ...
  async updateCard(cardId: number, cardData: UpdateCardData): Promise<Card> {
    const response = await getApiInstance().put<Card>(
      `/cards/${cardId}`,
      cardData,
    );
    return response.data;
  ...
  async archiveCard(cardId: number): Promise<Card> {
    const response = await getApiInstance().patch<Card>(
...
export const labelService = {
  ...
  async updateLabel(
    labelId: number,
    labelData: UpdateLabelData,
  ): Promise<Label> {
    const response = await getApiInstance().put<Label>(
      `/labels/${labelId}`,
      labelData,
    );
    return response.data;
...
export const boardSettingsService = {
  async getBoardTitle(): Promise<{ title: string }> {
    const response = await getApiInstance().get<{ title: string }>(
  ...
  async updateBoardTitle(title: string): Promise<{ title: string }> {
    const response = await getApiInstance().put("/board-settings/title", {
      title,
    });
...
export const cardItemsService = {
  async getItems(cardId: number): Promise<CardChecklistItem[]> {
    const response = await getApiInstance().get<CardChecklistItem[]>(
      `/card-items/card/${cardId}`,
    );
    return response.data;
  },
  async createItem(
    ...
    const response = await getApiInstance().post<CardChecklistItem>(
      "/card-items/",
      payload,
    );
    return response.data;
  ...
  async updateItem(
    itemId: number,
    data: Partial<Pick<CardChecklistItem, "text" | "is_done" | "position">>,
  ): Promise<CardChecklistItem> {
    const response = await getApiInstance().put<CardChecklistItem>(
      `/card-items/${itemId}`,
      data,
    );
    return response.data;
...
export const cardCommentsService = {
  async getComments(cardId: number): Promise<CardComment[]> {
    const response = await getApiInstance().get<CardComment[]>(
      `/card-comments/card/${cardId}`,
    );
    return response.data;
  },
  async createComment(cardId: number, comment: string): Promise<CardComment> {
    const response = await getApiInstance().post<CardComment>(
      "/card-comments/",
      { card_id: cardId, comment },
    );
    return response.data;
  },
  async updateComment(
    ...
    const response = await getApiInstance().put<CardComment>(
...
export const globalDictionaryService = {
  async getEntries(): Promise<GlobalDictionaryEntry[]> {
    const response = await getApiInstance().get<GlobalDictionaryEntry[]>(
      "/global-dictionary/",
    );
    return response.data;
  },
  ...
  async createEntry(entry: {
    ...
    const response = await getApiInstance().post<GlobalDictionaryEntry>(
      "/global-dictionary/",
      entry,
    );
    return response.data;
  ...
  async updateEntry(
    ...
    const response = await getApiInstance().put<GlobalDictionaryEntry>(
      `/global-dictionary/${entryId}`,
      entry,
    );
    return response.data;
...
export const personalDictionaryService = {
  async getEntries(): Promise<PersonalDictionaryEntry[]> {
    const response = await getApiInstance().get<PersonalDictionaryEntry[]>(
      "/personal-dictionary/",
    );
    return response.data;
  },
  ...
  async createEntry(entry: {
    ...
    const response = await getApiInstance().post<PersonalDictionaryEntry>(
      "/personal-dictionary/",
      entry,
    );
    return response.data;
  ...
  async updateEntry(
    ...
    const response = await getApiInstance().put<PersonalDictionaryEntry>(
      `/personal-dictionary/${entryId}`,
      entry,
    );
    return response.data;
