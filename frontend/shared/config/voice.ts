...
function isImportMetaEnv(env: unknown): env is ImportMetaEnv {
  return (
    typeof env === "object" &&
    env !== null &&
    "VITE_VOICE_TRANSCRIPT_LIMIT" in env
  );
}
...
const getTranscriptLimit = () => {
  ...
  if (
    ...
    isImportMetaEnv((import.meta as { env?: unknown }).env)
  ) {
    raw = ((import.meta as { env?: unknown }).env as ImportMetaEnv)
      .VITE_VOICE_TRANSCRIPT_LIMIT;
  }
  if (typeof raw === "string") {
    return normalizeLimit(raw);
  }
  if (typeof process !== "undefined") {
    const envValue = (process.env as Record<string, string | undefined>)
      ?.VITE_VOICE_TRANSCRIPT_LIMIT;
