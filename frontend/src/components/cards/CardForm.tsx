  ...
  return (
    <div ref={setNodeRef} style={style} className="touch-none">
      {typeof children === "function"
        ? (children as DragRender)({ attributes, listeners })
        : children}
    </div>
  );
};
...
const CardForm = ({
  ...
  const canEditCardContent = isEditing
    ? permissions.canModifyCardContent(card!)
    : canCreateCard;
  const canEditCardMetadata = isEditing
    ? permissions.canModifyCardMetadata(card!)
    : canCreateCard;
  ...
  const canToggleChecklistItems = isEditing
    ? permissions.canToggleCardItem(card!)
    : canCreateCard;

  // For EDITOR role, enforce self-assignment on creation
  const isEditorRole =
    permissions.isEditorOrAbove && !permissions.isSupervisorOrAbove;
  ...
  const ensureChecklistClientIds = (items: ChecklistItem[]): ChecklistItem[] =>
    items.map((item) =>
      item.id || item.clientId
        ? item
        : { ...item, clientId: `temp-${checklistIdCounter.current++}` },
    );
  ...
  const getFieldChangeInfo = (
    fieldName: string,
  ): { isChanged: boolean; tooltipContent: string } => {
    if (!proposedChanges) {
    ...
    switch (fieldName) {
      case "title": {
        ...
        const isChanged =
          oldValue !== newValue && proposedChanges.title !== undefined;
        return {
          isChanged,
      ...
      case "description": {
        ...
        const isChanged =
          oldValue !== newValue && proposedChanges.description !== undefined;
        return {
          isChanged,
          tooltipContent: t("voice.previousValue", {
            value: oldValue || t("common.empty"),
          }),
        };
      }
      case "due_date": {
        ...
        const isChanged =
          oldValue !== newValue && proposedChanges.due_date !== undefined;
        return {
          isChanged,
          tooltipContent: t("voice.previousValue", {
            value: oldValue || t("common.none"),
          }),
        };
      }
      case "priority": {
        ...
        const isChanged =
          oldValue !== newValue && proposedChanges.priority !== undefined;
        return {
          isChanged,
          tooltipContent: t("voice.previousValue", {
            value: oldValue ? t(`priority.${oldValue}`) : t("priority.medium"),
          }),
        };
      }
      case "assignee_id": {
        ...
        const isChanged =
          oldValue !== newValue && proposedChanges.assignee_id !== undefined;
        const oldUser = users.find((u) => u.id === oldValue);
        return {
          isChanged,
          tooltipContent: t("voice.previousValue", {
            value: oldUser?.display_name || t("card.unassign"),
          }),
        };
      }
      case "labels": {
        ...
        const isChanged =
          ...
          proposedChanges.label_ids !== undefined;
        const oldLabels = labels
          .filter((l) => oldIds.includes(l.id))
          .map((l) => l.name)
          .join(", ");
        return {
          isChanged,
          tooltipContent: t("voice.previousLabels", {
            labels: oldLabels || t("voice.noLabels"),
          }),
        };
      }
      case "list_id": {
        ...
        const isChanged =
          oldValue !== newValue && proposedChanges.list_id !== undefined;
        const oldList = lists.find((l) => l.id === oldValue);
        return {
          isChanged,
          tooltipContent: t("voice.previousValue", {
            value: oldList?.name || t("common.unknown"),
          }),
        };
      }
      default:
  ...
  const getChecklistItemChangeInfo = (
    item: ChecklistItem,
    index: number,
  ): {
    textChanged: boolean;
    ...
    return {
      ...
      statusTooltip: originalItem.is_done
        ? t("voice.wasChecked")
        : t("voice.wasUnchecked"),
      isNew: false,
    };
  };
    ...
    if (isOpen) {
      const run = async () => {
        ...
        if (card) {
          ...
          if (proposedChanges) {
            ...
            try {
              ...
              setOriginalValues((prev) => ({
                ...prev,
                checklist: originalItems,
              }));

              // Apply proposed changes to checklist
              if (proposedChanges.checklist) {
                setChecklist(
                  ensureChecklistClientIds(proposedChanges.checklist),
                );
              } else {
            ...
            setFormData({
              ...
              description:
                proposedChanges.description ?? (card.description || ""),
              due_date: proposedChanges.due_date ?? (card.due_date || ""),
              priority:
                proposedChanges.priority ??
                mapPriorityFromBackend(card.priority || ""),
              assignee_id:
                proposedChanges.assignee_id !== undefined
                  ? proposedChanges.assignee_id
                  : (card.assignee_id ?? null),
              label_ids:
                proposedChanges.label_ids ??
          ...
          const finalListId =
            effectiveListId > 0 ? effectiveListId : lists[0]?.id || -1;
          ...
          setFormData({
            ...
            assignee_id:
              ...
              (shouldEnforceSelfAssignment ? (currentUserId ?? null) : null),
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    if (
      shouldEnforceSelfAssignment &&
      (typeof formData.assignee_id !== "number" ||
        formData.assignee_id !== currentUserId)
    ) {
      toast({
    ...
    try {
      ...
      const labelIds = Array.isArray(formData.label_ids)
        ? formData.label_ids.map(Number)
        : [];
      const createPayload = {
        ...basePayload,
      ...
      try {
        for (let idx = 0; idx < checklist.length; idx++) {
          ...
          if (item.id) {
            await cardItemsService.updateItem(item.id, {
              text: item.text,
              is_done: item.is_done,
              position: idx + 1,
            });
          } else {
            await cardItemsService.createItem(
              savedCard.id,
              item.text,
              idx + 1,
              item.is_done,
        ...
        /* ignore checklist errors to not block card save */
      }
      onSave(savedCard);
      onClose();
      toast({
  ...
  const addChecklistItem = (): void => {
    ...
    setChecklist((prev) => [
      ...prev,
      { text, is_done: false, position: prev.length + 1, clientId },
    ]);
    setNewItemText("");
  };
  ...
  const toggleChecklistItem = async (index: number): Promise<void> => {
    ...
    if (item?.id && card?.id) {
      try {
        const updated = await cardItemsService.updateItem(item.id, {
          is_done: !item.is_done,
        });
        setChecklist((prev) =>
          prev.map((it, i) =>
            i === index ? { ...it, is_done: updated.is_done } : it,
          ),
        );
        // Also update the parent card's items in memory (without closing the form)
        if (card.items) {
          card.items = card.items.map((it) =>
            it.id === item.id ? { ...it, is_done: updated.is_done } : it,
          );
        }
      } catch (error: any) {
        toast({
          title: t("common.error"),
          description:
            error.response?.data?.detail || t("card.updateChecklistItemError"),
          variant: "destructive",
        });
      }
    } else {
      ...
      setChecklist((prev) =>
        prev.map((it, i) =>
          i === index ? { ...it, is_done: !it.is_done } : it,
        ),
      );
    }
  };
  ...
  const updateChecklistItemText = (index: number, text: string): void => {
    ...
    setChecklist((prev) =>
      prev.map((it, i) => (i === index ? { ...it, text: limited } : it)),
    );
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const oldIndex = checklist.findIndex(
      (i) => getChecklistItemKey(i) === active.id,
    );
    const newIndex = checklist.findIndex(
      (i) => getChecklistItemKey(i) === over.id,
    );
    if (oldIndex === -1 || newIndex === -1) {
      return;
    }
    const newItems = arrayMove(checklist, oldIndex, newIndex).map(
      (it, idx) => ({ ...it, position: idx + 1 }),
    );
  ...
  const handleDelete = async (): Promise<void> => {
    ...
    try {
      ...
      toast({
        ...
        description:
          error.response?.data?.detail || t("card.archiveErrorDescription"),
        variant: "destructive",
      });
    }
  };
      ...
      <DialogContent className="max-h-screen overflow-y-auto">
        <DialogHeader>
          ...
          <div className="flex items-center justify-between gap-3 w-full">
            <DialogDescription>
              {isViewOnly
                ? t("card.viewCardDescription")
                : card
                  ? t("card.editCardDescription")
                  : t("card.newCardDescription")}
            </DialogDescription>
            {/* List selector */}
            <HighlightedField
            ...
            >
              <Select
                value={
                  formData.list_id && formData.list_id > 0
                    ? formData.list_id.toString()
                    : lists[0]?.id.toString() || ""
                }
                onValueChange={(value) =>
                  setFormData((prev) => ({
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              >
                <SortableContext
                  items={checklist.map((item) => getChecklistItemKey(item))}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              <input
                                ...
                                disabled={
                                  isViewOnly && !canToggleChecklistItems
                                }
                              />
                            </HighlightedField>
                            <HighlightedField
                            ...
                            >
                              <Input
                                value={item.text}
                                onChange={(e) =>
                                  updateChecklistItemText(index, e.target.value)
                                }
                                className={
                                  item.is_done
              ...
              <div className="flex flex-wrap gap-2 p-2 rounded-md">
                {labels.map((label) => (
                  <Badge
                    ...
                    onClick={
                      isViewOnly ? undefined : () => handleLabelToggle(label.id)
                    }
                  >
                    {label.name}
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-2">
              ...
              <HighlightedField
                ...
                tooltipContent={
                  getFieldChangeInfo("assignee_id").tooltipContent
                }
              >
                <Select
