...
export const CardItem = ({
  ...
  const handleToggleChecklistItem = async (
    ...
    try {
      const updatedItem = await cardItemsService.updateItem(itemId, {
  ...
  const hiddenClass =
    isHidden || isActiveCard || isJustDropped ? "card-being-dragged" : "";
  ...
  return (
    ...
    >
      <CardContent
        className={isCompact ? "px-2" : ""}
        style={
          isCompact ? { paddingTop: "2px", paddingBottom: "2px" } : undefined
        }
      >
        <div>
          ...
          >
            <h3
              className={cn(
                "font-semibold leading-tight flex-1 text-foreground",
              ...
              <div className="flex items-center space-x-2">
                ...
                {card.due_date &&
                  (() => {
                    ...
                    return (
                      <div className={dateClasses} title={t("card.dueDate")}>
                        {isOverdue && (
                          <div className="absolute inset-0 bg-red-500/20 animate-pulse"></div>
                        )}
                        <div className="relative z-10 flex items-center">
                          <Icon
                            className={`h-3 w-3 mr-1 ${isOverdue ? "text-red-800" : isUpcoming ? "text-orange-600" : "text-muted-foreground"}`}
                          />
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <div
                        ...
                        title={t("card.checklistProgress")}
                      >
                    ...
                    <DropdownMenuContent
                      align="start"
                      className="w-72 p-0 bg-background border border-border"
                      sideOffset={5}
                    >
                        ...
                        <div className="space-y-1 max-h-64 overflow-y-auto">
                          {card.items?.map((item) => (
                            <div
                              ...
                              title={
                                canToggleChecklistItems
                                  ? t("card.toggleChecklistItem")
                                  : undefined
                              }
                            >
                              <div
                                className={`w-4 h-4 min-w-4 min-h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${item.is_done ? "bg-primary border-primary" : "border-muted-foreground"}`}
                              >
                                {item.is_done && (
                                  <svg
                                    className="w-2.5 h-2.5 text-primary-foreground flex-shrink-0"
                                    viewBox="0 0 24 24"
                                    fill="none"
                                    stroke="currentColor"
                                    strokeWidth="3"
                              ...
                              <span
                                className={`text-sm ${item.is_done ? "text-muted-foreground line-through" : "text-foreground"}`}
                              >
                                {item.text}
                              </span>
                            </div>
