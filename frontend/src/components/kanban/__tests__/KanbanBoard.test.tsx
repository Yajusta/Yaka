...
describe("KanbanBoard", () => {
  ...
  it("handles error when loading lists fails", async () => {
    ...
    vi.mocked(listsApi.listsApi.getLists).mockRejectedValue(
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Erreur lors du chargement du tableau"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows appropriate empty state when no lists exist", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Contactez un administrateur pour créer des listes"),
      ).toBeInTheDocument();
    });
  });
