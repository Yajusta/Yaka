...
export const KanbanBoard = ({
  ...
  const [justDroppedCardId, setJustDroppedCardId] = useState<number | null>(
    null,
  );
  ...
  const [, setAnimationData] = useState<
    Map<string, { element: HTMLElement; deltaY: number }>
  >(new Map());
  ...
  const calculateLayout = () => {
    ...
    const availableWidth =
      containerWidth - padding - (lists.length - 1) * gap - collapsedSpace;

    // Largeur par colonne si on utilise tout l'espace disponible (only for expanded columns)
    const widthPerColumn =
      numExpanded > 0 ? Math.floor(availableWidth / numExpanded) : 280;
  ...
  const calculatePositionFromMouse = (mouseX: number, mouseY: number): void => {
    ...
    if (closestColumn) {
      const listIdStr = (closestColumn as HTMLElement).getAttribute(
        "data-list-id",
      );
      if (listIdStr) {
        const listId = parseInt(listIdStr, 10);
  ...
  const handleDragOver = (event: DragOverEvent): void => {
    ...
    if (reportedOverId && reportedOverId !== "trash") {
      ...
      if (
        !isNaN(reportedListId) &&
        lists.some((list) => list.id === reportedListId)
      ) {
    ...
    if (activatorEvent) {
      ...
      if ("touches" in activatorEvent) {
        const touch =
          ...
          (activatorEvent as any).changedTouches?.[0];
        if (touch) {
          clientX = touch.clientX;
          clientY = touch.clientY;
        }
      ...
      if (clientX != null && clientY != null) {
        ...
        let listEl = elem?.closest
          ? (elem.closest("[data-list-id]") as HTMLElement | null)
          : null;
  ...
  const calculatePositionInList = (listId: number, mouseY: number): void => {
    ...
    if (columnElement) {
      ...
      if (cardsInList.length === 0) {
        ...
        if (mouseY < topArea + 50) {
          // Réduire la tolérance pour être plus précis
          insertPosition = 0;
        } else {
          ...
          const cardEls = Array.from(
            columnElement.querySelectorAll("[data-card-id]"),
          ) as HTMLElement[];
      ...
      if (
        !dropTarget ||
        dropTarget.listId !== listId ||
        dropTarget.position !== insertPosition
      ) {
        ...
        if (dropTarget && dropTarget.listId === listId) {
          if (
            ...
            Math.abs(mouseY - boundaryY) < HYSTERESIS
          ) {
  ...
  const handleDragEnd = async (event: DragEndEvent): Promise<void> => {
    ...
    if (finalDropTarget) {
      ...
      setTimeout(() => {
        ...
        try {
          delete (window as any).__SUPPRESS_ANIM_LIST_ID;
        } catch (e) {}
      }, 300);
      return;
    }
    ...
    >
      ...
      >
        ...
        <div className="h-full kanban-horizontal-scroll">
          <div
            className="kanban-lists-container h-full"
            style={{
