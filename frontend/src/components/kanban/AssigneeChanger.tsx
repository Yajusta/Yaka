...
export const AssigneeChanger: React.FC<AssigneeChangerProps> = ({
  card,
  onAssigneeChange,
  ...
  if (disabled) {
    return (
      <div
        ...
        title={t("card.changeAssignee")}
      >
        {card.assignee_id ? (
          <>
            <UserIcon
              className={`h-3 w-3 ${isCurrentUserAssigned ? "text-primary-foreground" : "text-muted-foreground"}`}
            />
            <span
              className={`text-xs font-medium ${isCurrentUserAssigned ? "text-primary-foreground" : "text-muted-foreground"}`}
            >
              {card.assignee_name || "-"}
            </span>
          </>
      ...
      <DropdownMenuTrigger asChild>
        ...
        >
          {card.assignee_id ? (
            ...
            <div
              className={`flex items-center ${isCurrentUserAssigned ? "text-primary-foreground hover:text-primary-foreground" : "text-muted-foreground hover:text-foreground"} transition-colors`}
            >
              <UserIcon className="h-3 w-3" />
            </div>
          )}
