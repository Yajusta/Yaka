...
export const PriorityBadge = ({ priority, className }: PriorityBadgeProps) => {
    const baseClass = 'inline-flex items-center justify-center p-0.5 rounded-sm cursor-pointer';
    ...
    const normalize = (p: string): 'low' | 'medium' | 'high' => {
        ...
        if (normalized.includes('medium') || normalized === 'medium') return 'medium';
        if (normalized.includes('low') || normalized === 'low') return 'low';

        // French forms (defensive)
        if (normalized.includes('elev') || normalized.includes('eleve') || normalized.includes('ele')) return 'high';
        if (normalized.includes('moy')) return 'medium';
        if (normalized.includes('faibl') || normalized.includes('faible')) return 'low';
