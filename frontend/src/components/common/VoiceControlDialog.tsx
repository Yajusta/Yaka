...
export const VoiceControlDialog = ({
  open,
  onOpenChange,
  onCardSave,
  defaultListId,
  ...
  const getInitialRecognitionMode = (): RecognitionMode => {
    try {
      ...
      if (
        saved &&
        ["browser", "whisper-tiny", "whisper-base"].includes(saved)
      ) {
        return saved as RecognitionMode;
      }
    } catch (error) {
      console.error(
        "Erreur lors du chargement du mode de reconnaissance:",
        error,
      );
    }
    return "browser";
  ...
  const isWhisperMode =
    recognitionMode === "whisper-tiny" || recognitionMode === "whisper-base";
  ...
  const handleModeChange = (mode: RecognitionMode) => {
    ...
    try {
      ...
      console.error(
        "Erreur lors de la sauvegarde du mode de reconnaissance:",
        error,
      );
    }
  ...
  if (isWhisperMode) {
    ...
    return (
      <VoiceControlWhisperDialog
        ...
        onSwitchModel={(model) => {
          const mode =
            model === "Xenova/whisper-tiny" ? "whisper-tiny" : "whisper-base";
