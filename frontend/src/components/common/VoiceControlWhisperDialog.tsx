...
import {
  ...
  CardFilterResponse,
} from "@shared/services/voiceControlApi";
...
export const VoiceControlWhisperDialog = ({
  ...
  const [result, setResult] = useState<
    VoiceControlResponse | CardFilterResponse | null
  >(null);
    ...
    const loadModel = async () => {
      ...
      try {
          ...
          selectedModel,
          {
            ...
            progress_callback: (progress: any) => {
              ...
              if (progress.status === "progress") {
                ...
                setLoadProgress(
                  `${t("voice.whisper.downloading")}: ${progress.file} (${percent}%)`,
                );
        ...
        if (error.message.includes("not valid JSON")) {
          errorMessage =
            "Erreur de chargement. Assurez-vous que l'application est servie via HTTP et non file://";
        }
  ...
  const transcribeAudio = async () => {
    ...
    try {
      ...
      if (newText) {
        ...
        const updatedTranscript = currentText
          ? currentText + " " + newText
          : newText;
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await voiceControlService.processTranscript(
        currentTranscript,
        voiceMode,
      );
      setResult(response);
      ...
      if (!cardResponse.task_id) {
        ...
        try {
          ...
          if (existingCard) {
            ...
            if (
              !permissions.canModifyCardContent(existingCard) &&
              !permissions.canModifyCardMetadata(existingCard)
            ) {
              toast({
                title: t("voice.noPermission"),
      ...
      console.error(
        "Erreur lors du traitement de l'instruction vocale:",
        error,
      );
      toast({
        title: t("common.error"),
    ...
    <>
          ...
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded">
                {initialModel === "Xenova/whisper-tiny"
                  ? "Whisper tiny"
                  : "Whisper"}
              </span>
            </DialogTitle>
          ...
          <div className="space-y-4">
            ...
            {modelLoadError && (
              <div className="flex items-center gap-2 text-sm text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/20 rounded-lg p-3">
                ...
                <span>
                  {t("voice.whisper.loadError")}: {modelLoadError}
                </span>
              </div>
            )}
              ...
              <div className="flex gap-4">
                ...
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setVoiceMode(e.target.value as "card_update")
                    }
                    className="w-4 h-4 text-primary focus:ring-primary border-gray-300"
                  />
              ...
              <div className="flex gap-2 items-center">
                {!isRecording ? (
                  <Button
                    ...
                    disabled={
                      isLoadingModel || isProcessing || !!modelLoadError
                    }
                  >
      ...
      {showCardForm && (
        <CardForm
          ...
          defaultListId={
            result && "list_id" in result && result.list_id != null
              ? result.list_id
              : defaultListId
          }
