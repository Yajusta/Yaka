...
const isFilterResponse = (
  result: VoiceControlResponse | CardFilterResponse | null,
): result is CardFilterResponse => {
  return Boolean(
    result && "response_type" in result && result.response_type === "filter",
  );
};
    ...
    return (
      <div className="rounded-md border border-dashed border-muted p-3 text-sm space-y-1">
        <div className="font-medium">
          {t("voice.filter.previewTitle", {
            defaultValue: "Last filter result",
          })}
        </div>
    ...
    <div className="rounded-md border border-muted/40 p-3 space-y-2 text-sm">
      <div className="flex items-center justify-between">
        <span className="font-medium">
          {result.title ||
            t("voice.card.noTitle", { defaultValue: "Untitled card" })}
        </span>
        {result.priority && (
