...
export default function UsersManager({
  ...
  const handleResendInvitation = async (userId: number) => {
    try {
      ...
      const errorMessage =
        e?.response?.data?.detail || t("user.invitationError");
      toast({
        title: t("common.error"),
  ...
  const handleSaveDisplayName = async () => {
    ...
    try {
      ...
      const errorMessage =
        e?.response?.data?.detail || t("user.updateNameError");
      toast({
        title: t("common.error"),
        description: errorMessage,
        variant: "destructive",
  ...
  const handleChangeRole = async (userId: number, newRole: UserRoleValue) => {
    ...
    try {
      ...
      const errorMessage =
        e?.response?.data?.detail || t("user.updateRoleError");
      toast({
        title: t("common.error"),
        description: errorMessage,
        variant: "destructive",
  ...
  const handleChangeViewScope = async (
    userId: number,
    ...
    try {
      ...
      const errorMessage =
        ...
        "Erreur lors de la mise à jour du périmètre de vue";
      toast({
        title: t("common.error"),
        description: errorMessage,
        variant: "destructive",
          ...
          <div className="space-y-4">
                  ...
                  <div className="divide-y">
                    <TooltipProvider>
                      {users.map((u) => (
                        <div
                          key={u.id}
                          className="group grid grid-cols-[1fr_1fr_auto_auto_auto_auto] gap-2 p-3 items-center hover:bg-muted/30"
                        >
                              ...
                              <TooltipTrigger asChild>
                                {(() => {
                                  const IconComponent = getRoleIcon(
                                    u.role as UserRoleValue,
                                  );
                                  return (
                                    <IconComponent
                                      className={`h-4 w-4 cursor-help ${u.role === "admin" ? "text-destructive" : "text-muted-foreground"}`}
                                    />
                                  );
                                })()}
                              </TooltipTrigger>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <div className="h-4 w-4 text-muted-foreground cursor-help">
                                  {(() => {
                                    const ViewScopeIconComponent =
                                      getViewScopeIcon(u.view_scope);
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                {(() => {
                                  ...
                                  if (status === "invited")
                                    return (
                                      <Mail className="h-4 w-4 text-yellow-600 cursor-help" />
                                    );
                                  return (
                                    <Check className="h-4 w-4 text-green-600 cursor-help" />
                                  );
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                    ...
                                    {roleOptions.map((option) => {
                                      ...
                                      return (
                                        <Tooltip key={option.value}>
                                          ...
                                          <TooltipContent
                                            side="right"
                                            className="p-3 bg-popover border-border"
                                          >
                                ...
                                <DropdownMenuSub>
                                  <Tooltip>
                                    ...
                                    <TooltipContent>
                                      <p>
                                        Modifier le périmètre de vue des cartes
                                      </p>
                                    </TooltipContent>
                                  </Tooltip>
                                        ...
                                        <Tooltip key={option.value}>
                                          <TooltipTrigger asChild>
                                            <div>
                                              <DropdownMenuItem
                                                onClick={() =>
                                                  handleChangeViewScope(
                                                    u.id,
                                                    option.value,
                                                  )
                                                }
                                                disabled={
                                                  u.view_scope === option.value
                                                }
                                                className="flex items-center gap-2"
                                              >
                                            ...
                                            <div className="flex flex-col gap-2">
                                              <div className="flex flex-col gap-1">
                                                ...
                                                <div className="flex items-center gap-2 text-xs">
                                                  ...
                                                  <span className="text-muted-foreground">
                                                    {t(
                                                      "viewScope.permissions.unassigned",
                                                    )}
                                                  </span>
                                                </div>
                                                <div className="flex items-center gap-2 text-xs">
                                                  {option.permissions.others ? (
                                                    <Check className="h-3 w-3 text-green-600" />
                                                  ) : (
                                                    <X className="h-3 w-3 text-red-600" />
                                                  )}
            ...
            {editingUser && (
              <div className="border-t pt-4 mt-4">
                <h3 className="font-medium text-sm mb-3">
                  {t("user.editName", { email: editingUser.email })}
                </h3>
