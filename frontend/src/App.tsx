...
import {
  DisplayModeProvider,
  useDisplayMode,
} from "@shared/hooks/useDisplayMode";
import "./index.css";
...
const KanbanApp = () => {
  ...
  const [showGlobalDictionary, setShowGlobalDictionary] =
    useState<boolean>(false);
  const [showPersonalDictionary, setShowPersonalDictionary] =
    useState<boolean>(false);
  const [listsRefreshTrigger, setListsRefreshTrigger] = useState<number>(0);
  const [defaultListIdForNewCard, setDefaultListIdForNewCard] = useState<
    number | null
  >(null);
  ...
  const filteredCards = useMemo(() => {
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter(
        (card) =>
          ...
          (card.description &&
            normalizeText(card.description).includes(searchTerm)),
      );
    }
    ...
    if (filters.assignee_id) {
      filtered = filtered.filter(
        (card) => card.assignee_id === filters.assignee_id,
      );
    }
  ...
  const handleCardDelete = async (cardId: number): Promise<void> => {
    ...
    try {
      ...
      toast({
        ...
        description:
          error.response?.data?.detail || t("app.archiveErrorDescription"),
        variant: "destructive",
      });
    }
  };
  ...
  const handleCardMove = async (
    ...
    try {
      ...
      if (position !== undefined && position >= 0) {
        const targetListCards = updatedCards.filter(
          (card) => card.list_id === newListId,
        );
        const movedCard = targetListCards.find((card) => card.id === cardId);

        if (movedCard) {
          // Remove the card from its current position in the array
          const cardsWithoutMoved = targetListCards.filter(
            (card) => card.id !== cardId,
          );
      ...
      toast({
        ...
        description:
          error.response?.data?.detail || t("app.moveErrorDescription"),
        variant: "destructive",
      });
    }
  };
        ...
        <KanbanBoard
          ...
          onCreateCard={
            permissions.canCreateCard
              ? (listId) => handleCreateCard(listId)
              : undefined
          }
