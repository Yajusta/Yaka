...
describe("Simple Integration Tests", () => {
  ...
  it("should test complete list management workflow", async () => {
    ...
    expect(mockListsApi.createList).toHaveBeenCalledWith({
      name: "New List",
      order: 4,
    });
    expect(mockListsApi.updateList).toHaveBeenCalledWith(4, {
      name: "Updated List",
    });
    expect(mockListsApi.deleteList).toHaveBeenCalledWith(4, 1);
  });
  ...
  it("should test error handling workflow", async () => {
    ...
    await expect(listsApi.createList({ name: "", order: 1 })).rejects.toThrow(
      "Validation error",
    );
  ...
  it("should test complete user journey simulation", async () => {
    ...
    const reviewList = await listsApi.createList({
      name: "Code Review",
      order: 4,
    });
    ...
    expect(mockListsApi.reorderLists).toHaveBeenCalledWith({
      1: 1,
      2: 2,
      4: 3,
      3: 4,
