...
export const validateListName = (name: string): ValidationResult => {
  ...
  if (invalidChars.test(trimmedName)) {
    return {
      isValid: false,
      error:
        "Le nom de la liste contient des caractères non autorisés (<, >, \", ')",
    };
  }
...
export const isListNameUnique = (
  name: string,
  existingLists: Array<{ name: string; id?: number }>,
  excludeId?: number,
): ValidationResult => {
...
export const validateListDeletion = (
  ...
  if (cardCount > 0 && !targetListId) {
    return {
      isValid: false,
      error:
        "Vous devez sélectionner une liste de destination pour les cartes existantes",
    };
  }
 ...
 */
export const validateListReorder = (
  listOrders: Record<number, number>,
): ValidationResult => {
...
export const getErrorMessage = (error: any): string => {
  ...
  switch (error.response.status) {
    ...
    case 422:
      return (
        error.response.data?.detail || "Données de validation incorrectes."
      );
    case 500:
    ...
    default:
      return (
        error.response.data?.detail || "Une erreur inattendue s'est produite."
      );
  }
};
