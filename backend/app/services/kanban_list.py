...
from ..models import KanbanList, Card
...
class KanbanListService:
    ...
    def get_list_with_cards_count(db: Session, list_id: int) -> Tuple[Optional[KanbanList], int]:
        ...
        try:
            ...
            cards_count = db.query(Card).filter(
                Card.list_id == list_id,
                Card.is_archived == False
            ).count()
            return kanban_list, cards_count
        except Exception as e:
            raise ValueError(f"Erreur lors du comptage des cartes: {str(e)}")
    ...
    def create_list(db: Session, list_data: KanbanListCreate) -> KanbanList:
        ...
        """
        # Validation du nom - vérifier l'unicité (case-insensitive)
        existing_list = db.query(KanbanList).filter(
            func.lower(KanbanList.name) == func.lower(list_data.name)
        ).first()
        if existing_list:
        ...
        if total_lists >= 50:
            raise ValueError("Nombre maximum de listes atteint (50). Supprimez des listes existantes avant d'en créer de nouvelles.")
        
        # Vérifier si l'ordre existe déjà et ajuster si nécessaire
        existing_order = db.query(KanbanList).filter(KanbanList.order == list_data.order).first()
        if existing_order:
        ...
        try:
            ...
            db.rollback()
            raise ValueError(f"Erreur lors de la création de la liste: {str(e)}")
    ...
    def update_list(db: Session, list_id: int, list_data: KanbanListUpdate) -> Optional[KanbanList]:
        ...
        if "order" in update_data:
            ...
            if new_order != old_order:
                # Vérifier si le nouvel ordre existe déjà
                existing_order = db.query(KanbanList).filter(
        ...
        try:
            ...
            db.rollback()
            raise ValueError(f"Erreur lors de la mise à jour de la liste: {str(e)}")
    ...
    def delete_list(db: Session, list_id: int, target_list_id: int) -> bool:
        ...
        total_lists = db.query(KanbanList).count()
        if total_lists <= 1:
            raise ValueError("Impossible de supprimer la dernière liste. Au moins une liste doit exister dans le système.")
        ...
        try:
            ...
            if cards_count > 0:
                cards_moved = db.query(Card).filter(Card.list_id == list_id).update(
                    {Card.list_id: target_list_id}
                )
                
                if cards_moved != cards_count:
                    raise ValueError(f"Erreur lors du déplacement des cartes: {cards_moved} cartes déplacées sur {cards_count} attendues")
            ...
            if isinstance(e, ValueError):
                ...
                raise ValueError(f"Erreur lors de la suppression de la liste: {str(e)}")
    ...
    def _shift_orders_up(db: Session, from_order: int) -> None:
        ...
        db.query(KanbanList).filter(KanbanList.order >= from_order).update(
            {KanbanList.order: KanbanList.order + 1}
        )
