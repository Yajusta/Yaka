...
async def create_label(
    ...
    if label is None:
        raise HTTPException(
            ...
            detail="Données invalides pour la création du libellé",
        )
    try:
    ...
    try:
        return label_service.create_label(
            db=db, label=label, created_by=current_user.id
        )
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)
...
async def read_label(
    ...
    if db_label is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Libellé non trouvé"
        )
    return db_label
...
async def update_label(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Identifiant de libellé invalide",
        ) from exc
    if label_update is None:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Données invalides pour la mise à jour du libellé",
        )
    try:
        db_label = label_service.update_label(
            db, label_id=label_id, label_update=label_update
        )
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)
