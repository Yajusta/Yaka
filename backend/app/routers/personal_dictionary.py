...
async def read_entries(
    ...
    return personal_dictionary_service.get_entries_by_user(
...
async def create_entry(
    ...
    if db_entry:
        raise HTTPException(
            ...
            detail="You already have an entry with this term",
        )
    try:
        return personal_dictionary_service.create_entry(
            db=db, entry=entry, user_id=current_user.id
        )
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)
...
async def read_entry(
    ...
    if db_entry.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only access your own dictionary entries",
        )
...
async def update_entry(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Invalid dictionary entry ID",
        ) from exc
    ...
    if db_entry.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only update your own dictionary entries",
        )

    try:
        db_entry = personal_dictionary_service.update_entry(
            db, entry_id=entry_id, entry_update=entry_update
        )
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)
