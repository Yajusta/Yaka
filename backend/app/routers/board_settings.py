...
async def update_board_title(
    ...
    if not title_update.title or not title_update.title.strip():
        raise HTTPException(
            ...
            detail="Le titre ne peut pas etre vide",
        )
...
async def read_board_setting(
    ...
    if setting := board_settings_service.get_setting(db, setting_key):
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Paramètre non trouvé"
        )
...
async def update_board_setting(
    ...
    if not setting_value:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="setting_value est requis"
        )

    return board_settings_service.create_or_update_setting(
        db, setting_key, setting_value, description
    )
