...
@pytest.mark.asyncio
async def test_password_reset_request_invited_user(
    ...
    async with async_client_factory(auth_router) as client:
        ...
        try:
            ...
            assert (
                updated_user.status == UserStatus.INVITED
            )  # Status should remain INVITED
            assert updated_user.invite_token is not None
            assert (
                updated_user.invite_token != original_token
