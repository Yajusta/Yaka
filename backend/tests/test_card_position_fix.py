...
def test_user(db_session):
    ...
    user = User(
        email="test@example.com", password_hash="test", display_name="Test User"
    )
    db_session.add(user)
    db_session.commit()
...
class TestCardPositionFix:
    ...
    def test_new_cards_get_correct_positions(self, db_session, test_user, test_list):
        ...
        # Créer 3 cartes
        card1 = create_card(
            db_session, CardCreate(title="Carte 1", list_id=test_list.id), test_user.id
        )

        card2 = create_card(
            db_session, CardCreate(title="Carte 2", list_id=test_list.id), test_user.id
        )

        card3 = create_card(
            db_session, CardCreate(title="Carte 3", list_id=test_list.id), test_user.id
        )
    ...
    def test_cards_are_retrieved_in_position_order(
        self, db_session, test_user, test_list
    ):
        """Test que les cartes sont récupérées dans l'ordre de position."""
        # Créer 3 cartes
        create_card(
            db_session, CardCreate(title="Carte A", list_id=test_list.id), test_user.id
        )
        create_card(
            db_session, CardCreate(title="Carte B", list_id=test_list.id), test_user.id
        )
        create_card(
            db_session, CardCreate(title="Carte C", list_id=test_list.id), test_user.id
        )
    ...
    def test_move_card_to_first_position_same_list(
        self, db_session, test_user, test_list
    ):
        ...
        move_card(
            ...
            card3.id,
            CardMoveRequest(
                source_list_id=test_list.id, target_list_id=test_list.id, position=0
            ),
        )
        ...
        assert positions == [
            ...
            1,
            2,
        ], f"Positions attendues: [0, 1, 2], obtenues: {positions}"
    ...
    def test_move_card_to_last_position_same_list(
        self, db_session, test_user, test_list
    ):
        ...
        move_card(
            ...
            card1.id,
            CardMoveRequest(
                source_list_id=test_list.id, target_list_id=test_list.id, position=2
            ),
        )
    ...
    def test_positions_are_normalized_after_card_deletion(
        self, db_session, test_user, test_list
    ):
        ...
        card4 = create_card(
            db_session, CardCreate(title="Carte 4", list_id=test_list.id), test_user.id
        )
    ...
    def test_move_card_up_without_duplicate_positions(
        self, db_session, test_user, test_list
    ):
        ...
        assert positions == [
            ...
            2,
            3,
        ], f"Positions initiales incorrectes: {positions}"
        ...
        move_card(
            ...
            card4.id,
            CardMoveRequest(
                source_list_id=test_list.id, target_list_id=test_list.id, position=1
            ),
        )
        ...
        ), f"Ordre attendu: {expected_order}, obtenu: {actual_order}"

    def test_archived_cards_dont_affect_positions(
        self, db_session, test_user, test_list
    ):
        ...
        cards = get_cards(
            db_session, CardFilter(list_id=test_list.id, include_archived=False)
        )
    ...
    def test_move_card_between_lists_with_specific_position(
        self, db_session, test_user, test_list
    ):
        ...
        card1 = create_card(
            db_session, CardCreate(title="Source 1", list_id=test_list.id), test_user.id
        )
        card2 = create_card(
            db_session, CardCreate(title="Source 2", list_id=test_list.id), test_user.id
        )
        card3 = create_card(
            db_session, CardCreate(title="Source 3", list_id=test_list.id), test_user.id
        )

        # Créer 3 cartes dans la liste cible
        card4 = create_card(
            db_session,
            CardCreate(title="Target 1", list_id=target_list.id),
            test_user.id,
        )
        card5 = create_card(
            db_session,
            CardCreate(title="Target 2", list_id=target_list.id),
            test_user.id,
        )
        card6 = create_card(
            db_session,
            CardCreate(title="Target 3", list_id=target_list.id),
            test_user.id,
        )
        ...
        move_card(
            ...
            card2.id,
            CardMoveRequest(
                source_list_id=test_list.id, target_list_id=target_list.id, position=1
            ),
        )
