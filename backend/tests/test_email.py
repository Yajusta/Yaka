...
class TestSendMail:
    ...
    def test_send_mail_success_starttls(self, mock_smtp, sample_email_data):
        ...
        with patch('app.services.email.SMTP_SECURE', 'starttls'), \
             patch('app.services.email.SMTP_USER', 'test@example.com'), \
    ...
    def test_send_mail_success_none(self, mock_smtp, sample_email_data):
        ...
        with patch('app.services.email.SMTP_SECURE', 'none'), \
             patch('app.services.email.SMTP_USER', ''), \
    ...
    def test_send_mail_message_content(self, mock_smtp, sample_email_data):
        ...
        with patch('app.services.email.FROM_ADDRESS', 'test@example.com'):
            ...
            if plain_body:
                ...
                assert sample_email_data['plain_body'].strip() in content or content == sample_email_data['plain_body'].strip()
...
class TestEmailConfiguration:
    ...
    def test_smtp_secure_case_insensitive(self):
        """Test que SMTP_SECURE est insensible à la casse."""
        ...
        try:
            ...
            # Si une erreur se produit, elle ne devrait pas être liée à la casse
            pass
...
class TestEmailIntegration:
    ...
    def test_concurrent_email_sending(self, mock_smtp):
        ...
        emails_data = [
            {'to': f'user{i}@example.com', 'subject': f'Subject {i}', 'html_body': f'<p>Content {i}</p>', 'plain_body': f'Content {i}'}
